<template>
  <div id="prismic-tutorial">
    <header>
      ...
      <div class="wrapper">
        ...
        <p>
          ...
          component with content managed in Prismic.
        </p>
      </div>
    ...
    <section>
      <p>
        This is a tutorial page included in this Vue.js starter project, it has a few useful links
        and example code snippets to help you get started.
      </p>
      <p>
        You can access this very tutorial page at
        <a href="http://localhost:8080/tutorial">http://localhost:8080/tutorial</a>.
      </p>
...
&lt;/script&gt;
      ...
      <p>
        ...
        UID field (unique identifier) for querying the document.
      </p>
      <p>
        Go to the Web interface of the Prismic repository you’ve just created (at
        https://your-repo-name.prismic.io). Then navigate to the <em>"Custom Types"</em> section
        (icon on the left navbar) and create a new Repeatable Type, for this tutorial let’s name it
        "Page", so that its API ID will be set to <strong>"page"</strong>.
      </p>
      <p>
        Once the "Page" Custom Type is created, we have to define how we want to model it, that is
        to say a document containing a UID field, a rich text field for the title, a rich text field
        for the description, a link field + a rich text field for the call to action and an image
        field for the icon. Click on <em>"JSON editor"</em> (right sidebar) and paste in the
        following JSON data.
      </p>
  ...
  "Main": {
    ...
    "icon": {
      ...
      <p>
        Create a new "Page" document in your repository: go to <em>"Content"</em> and hit
        <em>"New"</em>. Fill the corresponding fields. Note the value you filled in the UID field,
        because it will be a part of the component route path, for this tutorial let’s put
      ...
      <p>
        Go back to your local code and create a new Vue component file named "Page.vue" inside the
        views folder (src/views/Page.vue). We’ll make an API call to retrieve the document content,
        querying it by specifying its UID.
      </p>
      <p>
        Here’s an example that’ll render a "Page" document with its title, description, call to
        action and icon:
      </p>
      <pre
        v-highlightjs
        class="source-code"
      ><code class="xml">&lt;!-- Create file src/views/Page.vue --&gt;
...
&lt;/style&gt;
      ...
      <p>
        Now that you’ve created your Page component, add the following route to the Vue Router, in
        src/router.js file:
      </p>
...
// ...
      ...
      <ol style="list-style-type: disc; padding-left: 20px;">
        ...
        <li>
          Performance and scalability for your content using a Content Delivery Network around the
          world
        </li>
      </ol>
    </section>
